...
**Fase 2 (pareamento greedy de catch-all, `dc_pairer._parear_catchall`) tem cobertura zero em dado real nos casos travados** — os 6 casos acima são todos de classificação, nenhum exercita o pareamento N×M. SGF (o catch-all real conhecido) falha antes, na classificação (`score_baixo`), nunca chega no `dc_pairer` no dataset GTD atual — por isso não há caso real disponível pra travar ainda. Risco: mudança futura em `descricoes.normalizada` ou no limiar (`config.limiar_pareamento_similaridade`, default 60.0, não calibrado) pode alterar pareamento sem nenhum gate real pegando. Quando um catch-all real chegar no `dc_pairer` (ex. se a Fase 1 destravar a classificação de SGF), adicionar esse caso a `bench/casos_travados.csv`.
